...
// Read sink configuration from file
    FILE *fp;
    uint32_t network_address;
    uint32_t sink_address;
    char *config_file = "/home/pi/wirepas/network_id";
    ...
    fscanf(fp, "%x", &network_address);
    fscanf(fp, "%x", &sink_address);
    fclose(fp);

    if (!get_service_name(full_service_name, sink_address))
    {
        return EXIT_FAILURE;
    }
    ...
    if (WPC_set_node_address(sink_address) != APP_RES_OK)
    {
        LOGE("Cannot set node address\n");
        goto finish;
    }
    LOGI("Node address set : %x\n", sink_address);
