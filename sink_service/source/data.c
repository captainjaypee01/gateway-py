                           ...
                           unsigned long long timestamp_ms)
{
    ...
    int r;
    uint8_t offset = 0;
    uint8_t num_size = 2 * num_bytes;
    char dataStr[2 * num_bytes + 1];

    for(size_t i = 0; i < num_bytes; i++)
        offset += sprintf(dataStr + offset, "%02x", bytes[i]);

    // struct msg new_msg;
    // char *json;
    // new_msg.len = asprintf(&json,
    //         "{\"NodeID\":\"%x\",\"System\":%d,\"EP\":%d,\"Data\":\"%s\",\"Timestamp\":%llu,\"Travel\":%d,\"Hop\":%d}",
    //         src_addr, src_addr, dst_addr, dataStr, timestamp_ms, travel_time, hop_count);
    // new_msg.payload = malloc(new_msg.len);
    // //LOGD("Storing %d bytes - %s\n",new_msg.len,json);
    // if (!new_msg.payload) {
    //     LOGE("Memory is full!\n");
    //     free(json);
    //     return false;
    // } else {
    //     memcpy((char *) new_msg.payload, json, new_msg.len);
        
    // }
    
    LOGD("%llu -> Data received on EP %d of len %d from 0x%x to 0x%x Data(%d)\n",
         timestamp_ms,
         dst_ep,
         num_bytes,
         src_addr,
    ...
    r = sd_bus_message_append_array(m, 'y', dataStr, num_size);
    if (r < 0)
    {
